...
class WsAgent {
    constructor(options) {
        ...
        if (!options.disableNonce) {
            this.addTransform((0, transforms_1.makeWsNonceTransform)(agent_1.makeNonce));
        }
    }
    ...
    async getPrincipal() {
        if (!this._identity) {
            throw new agent_1.IdentityInvalidError("This identity has expired due this application's security policy. Please refresh your authentication.");
        }
    ...
    async call(canisterId, options) {
        ...
        let ingress_expiry = new agent_1.Expiry(DEFAULT_INGRESS_EXPIRY_DELTA_IN_MSECS);
        // If the value is off by more than 30 seconds, reconcile system time with the network
        if (Math.abs(this._timeDiffMsecs) > 1000 * 30) {
            ingress_expiry = new agent_1.Expiry(DEFAULT_INGRESS_EXPIRY_DELTA_IN_MSECS + this._timeDiffMsecs);
        }
        const submit = {
            request_type: agent_1.SubmitRequestType.Call,
    ...
    _requestAndRetry(message, tries = 0) {
        const messageBytes = agent_1.Cbor.encode(message);
        try {
