import { Cbor, Certificate, compare, lookup_path, lookupResultToBuffer, reconstruct, } from "@dfinity/agent";
import { logger } from "./logger";
const areBuffersEqual = (buf1, buf2) => {
    return compare(buf1, buf2) === 0;
};
export const isMessageBodyValid = async (canisterId, path, body, certificate, tree, agent, maxCertificateAgeInMinutes) => {
    let cert;
    try {
    ...
    if (!treeSha) {
        ...
        logger.error(`[certification] Invalid Tree in the header. Does not contain path ${JSON.stringify(path)}`);
        return false;
    }
    return !!treeSha && areBuffersEqual(sha, treeSha);
};
export const safeExecute = async (fn, warnMessage) => {
    try {
